...
void Graph::BFS(int departure, set<string> &airlines) {
    ...
    while (!visitedNodes.empty()) {
        ...
        if (nodes[node].available) {
            for (const Edge &edge : nodes[node].adjacent) {
                ...
                if (!nodes[n].visited) {
                    if(airlines.find(edge.flight.airline) != airlines.end()) {
                        visitedNodes.push(n);
                        nodes[n].visited = true;
                        nodes[n].distance = nodes[node].distance + 1;
                        nodes[n].parent = node;
...
vector<Node> Graph::makePath(int departure, int arrival, set<string> &airlines) {
    ...
    if(nodes[arrival].currentFlight.airline == " ") {
        cout << "Não existem voos possiveis com estas configuracoes" <<endl;
        return path;
    }
